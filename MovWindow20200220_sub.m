function MovWindow20200220_sub
...
if length(LlocsInterval)>WindowNum && length(RlocsInterval)>WindowNum...
   ...
   %%%%%%%%左脚のピークとトラフの関係を調べる
   %トラフ間のピークが複数ある場合の修正
    for i=1:length(LTroughlocsInterval)
   ...
   %%%%%%%%右脚のピークとトラフの関係を調べる
   %トラフ間のピークが複数ある場合の修正
    for i=1:length(RTroughlocsInterval)
...
LtouchWindowPeak=sort(LlocsInterval(LlocsIntervalIndex(1:WindowNum)),'ascend');        %%%%WindowNum個分のピーク

%Rtouch
[RlocsIntervalB RlocsIntervalIndex]=sort(abs(RlocsInterval),'ascend');
RtouchWindowPeak=sort(RlocsInterval(RlocsIntervalIndex(1:WindowNum)),'ascend');        %%%%WindowNum個分のピーク
...
for n=1:WindowNum
    ...
    LPWS(n)=LtouchWindowPeak(n)-abs(max(DistancefromPeakNLeft(1)))/2;%Peakの左隣にあるTroughとの半分の距離をwindowの端（windowstart）に設定
    LPWE(n)=LtouchWindowPeak(n)+abs(min(DistancefromPeakNRight(1)))/2;%Peakの右隣にあるTroughとの半分の距離をwindowの端に(windowEnd)に設定
end
...
% X=sort(XX)
% Y=sort(YY)
% LNWSsort=sort(LNWS);%aka
% LNWEsort=sort(LNWE);%akafilled
% LPWSsort=sort(LPWS);%ao 
% LPWEsort=sort(LPWE);%aofilled
% figure
% plot(linspace(1,5000,bin),CCresultLtouchWon,'color','r','LineWidth',2);hold on 
% axis([0 5000 0 max(CCresultLtouchWon)*1.1]);
% scatter(LNWSsort*10+2500,[0.002,0.002,0.002,0.002],'r');hold on 
% scatter(LNWEsort*10+2500,[0.002,0.002,0.002,0.002],'r','filled');hold on 
% scatter(LPWSsort*10+2500,[0.0022,0.0022,0.0022,0.0022,0.0022],'b');
% scatter(LPWEsort*10+2500,[0.0022,0.0022,0.0022,0.0022,0.0022],'b','filled');
% 
% %%%%%%%%off


%%%%%%ウィンドウ１のスターとの位置を０ｓとする値に変換（LPWS(1)orLPWS(1)分、値をずらす）
LPWS1=LPWS-LPWS(1);
LPWE1=LPWE-LPWS(1);
LNWS1=LNWS-LPWS(1);
LNWE1=LNWE-LPWS(1);
...
for n=1:MoveWindowNum
    LPWStart=LPWS1+LpegTouchallWon(1)+(n-1)*10;
    LPWEnd=LPWE1+LpegTouchallWon(1)+(n-1)*10;
    LNWStart=LNWS1+LpegTouchallWon(1)+(n-1)*10;
    LNWEnd=LNWE1+LpegTouchallWon(1)+(n-1)*10;
    SWSart=LSWS1+LpegTouchallWon(1)+(n-1)*10;
    SWEnd=LSWE1+LpegTouchallWon(1)+(n-1)*10;
...
for n=1:MoveWindowNum
    RPWStart=RPWS1+RpegTouchallWon(1)+(n-1)*10;
    RPWEnd=RPWE1+RpegTouchallWon(1)+(n-1)*10;
    RNWStart=RNWS1+RpegTouchallWon(1)+(n-1)*10;
    RNWEnd=RNWE1+RpegTouchallWon(1)+(n-1)*10;
    SWSart=RSWS1+RpegTouchallWon(1)+(n-1)*10;
    SWEnd=RSWE1+RpegTouchallWon(1)+(n-1)*10;
...
filename=['SpikeOnOffRtLtUnit.mat'];
save(filename,'RtouchSpikeOnUnit','LtouchSpikeOnUnit','RtouchSpikeOffUnit','LtouchSpikeOffUnit');
