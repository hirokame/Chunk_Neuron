function Rtseq5Touch
%ピーク間の4つのインターバルの平均をとって、3つ目のウインドウを基準にして平均のインターバルでウインドウを設定する
global CCresultRtouchWon CCresultLtouchWon RpegTouchallWon LpegTouchallWon SpikeArrayWon tfile pegpatternname trimFolder2 pegpatternum RtFR_Array
...
if length(Linterval1)>2 && length(Linterval2)>2 && length(Rinterval1)>2 && length(Rinterval2)>2;  
    ...
    for n=6:length(RpegTouchallWon)-5
        W0=0;
        W1=0;
        W2=0;
        W3=0;
        ...
        W8=0;
        W9=0;
       if  RpegTouchallWon(n)-IntervalAverage*10-90<RpegTouchallWon(n-1) && RpegTouchallWon(n-1)<RpegTouchallWon(n)-IntervalAverage*10+90
           W4=1;
           if RpegTouchallWon(n-1)-IntervalAverage*10-90<RpegTouchallWon(n-2) && RpegTouchallWon(n-2)<RpegTouchallWon(n-1)-IntervalAverage*10+90
               W3=1;
               if RpegTouchallWon(n-2)-IntervalAverage*10-90<RpegTouchallWon(n-3) && RpegTouchallWon(n-3)<RpegTouchallWon(n-2)-IntervalAverage*10+90
                   W2=1;
                   if RpegTouchallWon(n-3)-IntervalAverage*10-90<RpegTouchallWon(n-4) && RpegTouchallWon(n-4)<RpegTouchallWon(n-3)-IntervalAverage*10+90
                       W1=1;
                       if RpegTouchallWon(n-4)-IntervalAverage*10-90<RpegTouchallWon(n-5) && RpegTouchallWon(n-5)<RpegTouchallWon(n-4)-IntervalAverage*10+90
                           W0=1;
                       end
                   end
       ...
       if W3==1 && W4==1 && W5==1 && W6==1 %ウインドウ5連続で当てはまっている
           seq5time=[RpegTouchallWon(n-2),RpegTouchallWon(n-1),RpegTouchallWon(n),RpegTouchallWon(n+1),RpegTouchallWon(n+2)];
           seq5timeArray=[seq5timeArray;seq5time];
       end
       if W3==1 && W4==1 && W5==1 && W6==1 && W2==0 && W7==0 %ウインドウ5連続で当てはまっていて、両隣は当てはまっていない
           seqonly5time=[RpegTouchallWon(n-2),RpegTouchallWon(n-1),RpegTouchallWon(n),RpegTouchallWon(n+1),RpegTouchallWon(n+2)];
           seqonly5timeArray=[seqonly5timeArray;seqonly5time];
       end
       if W2==1 && W3==1 && W4==1 && W5==1 && W6==1 && W7==1%ウインドウ7連続で当てはまっている
           seq7time=[RpegTouchallWon(n-3),RpegTouchallWon(n-2),RpegTouchallWon(n-1),RpegTouchallWon(n),RpegTouchallWon(n+1),RpegTouchallWon(n+2),RpegTouchallWon(n+3)];
           seq7timeArray=[seq7timeArray;seq7time];
       end
       if W2==1 && W3==1 && W4==1 && W5==1 && W6==1 && W7==1 && W1==0 && W8==0 %ウインドウ7連続で当てはまっていて、両隣当てはまっていない
           seqonly7time=[RpegTouchallWon(n-3),RpegTouchallWon(n-2),RpegTouchallWon(n-1),RpegTouchallWon(n),RpegTouchallWon(n+1),RpegTouchallWon(n+2),RpegTouchallWon(n+3)];
           seqonly7timeArray=[seqonly7timeArray;seqonly7time];
       end
       if W1==1 && W2==1 && W3==1 && W4==1 && W5==1 && W6==1 && W7==1 && W8==1 %ウインドウ9連続で当てはまっている
           seq9time=[RpegTouchallWon(n-4),RpegTouchallWon(n-3),RpegTouchallWon(n-2),RpegTouchallWon(n-1),RpegTouchallWon(n),RpegTouchallWon(n+1),RpegTouchallWon(n+2),RpegTouchallWon(n+3),RpegTouchallWon(n+4)];
           seq9timeArray=[seq9timeArray;seq9time];
       end
       if W1==1 && W2==1 && W3==1 && W4==1 && W5==1 && W6==1 && W7==1 && W8==1 && W0==0 && W9==0 %ウインドウ9連続で当てはまっていて、両隣当てはまっていない
           seqonly9time=[RpegTouchallWon(n-4),RpegTouchallWon(n-3),RpegTouchallWon(n-2),RpegTouchallWon(n-1),RpegTouchallWon(n),RpegTouchallWon(n+1),RpegTouchallWon(n+2),RpegTouchallWon(n+3),RpegTouchallWon(n+4)];
    ...
    RtFR_Array=[seq5time_FR,seqonly5time_FR,seq7time_FR,seqonly7time_FR,seq9time_FR,seqonly9time_FR];
    fig1=figure;
    bar(RtFR_Array);
    set(gca,'xticklabel',{'5seq','5only','7seq','7only','9seq','9only'});
    trimtfile=strtrim(tfile);
    figname=['RtseqFR',trimFolder2,trimtfile,pegpatternum,pegpatternname,'.bmp'];
    saveas(fig1,figname);
    close(fig1); 
