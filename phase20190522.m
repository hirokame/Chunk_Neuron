function phase20190522
%スパイクでアラインしたタッチのヒストグラムから左に対する右足の位相を計算する
...
PhaseIndex=(RtPeakInLt-LtPeakArray(1))/(LtPeakArray(2)-LtPeakArray(1));
else
    PhaseIndex=0;
end
if isempty(PhaseIndex)==1
   PhaseIndex=0; 
end
if length(PhaseIndex)>1
   PhaseIndex=mean(PhaseIndex); 
end
