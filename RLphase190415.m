...
global  ACresult ACresultW CCresultDrinkOn SpikeArray TurnMarkerTime RpegTouchallWon LpegTouchallWon...
    ...
    StartTime FinishTime DiffMaxMinR DiffMaxMinL fname tfile 
%%すべてのタッチに位相を当てはめてからターンマーカーでアラインしたバージョン
%一歩の間の位相に中央値の閾値をつけて足し算するバージョン
% RmedianPTTM
% LmedianPTTM
...
MedianR=median(RAllTouchPhase);
MedianL=median(LAllTouchPhase);
RAllTouchPhase=RAllTouchPhase-MedianR;
LAllTouchPhase=LAllTouchPhase-MedianL;
...
subplot(2,1,2)
plot(linspace(0,OneTurnTime98*2,length(RLPhaseTM)),RLPhaseTM);hold on 
if max(RLPhaseTM)>0;
axis([0 OneTurnTime98*2 0 max(RLPhaseTM)*1.1]);
end
ylabel('median');
Old1=cd;
...
% close
cd(Old2);
figname=[tfile(1:end-2), 'onestepphasemedian','.bmp'];
saveas(y,figname);
