function Peak_Variance_ALLTouch_20201208
    ...
    if ~strcmp(TrimFolder,'.') && ~strcmp(TrimFolder,'..') && isempty(strfind(TrimFolder,'.mat')) && isempty(strfind(TrimFolder,'.bmp'));
        ...
        for j=1:length(LS1(:,1))
            ...
            if length(TrimFolder2)>22 && strcmp(TrimFolder2(end-21:end),'SpikeOnOffRtLtUnit.mat');
                ...
                if length(Random5seqSetLtouch)<length(Seq5LtouchTimeunique);
                    continue
                end
                randomIndex= randi([1 length(Random5seqSetLtouch)],1,length(Seq5LtouchTimeunique));
                ...
                if length(Random5seqSetRtouch)<length(Seq5RtouchTimeunique);
                    continue
                end
                randomIndex= randi([1 length(Random5seqSetRtouch)],1,length(Seq5RtouchTimeunique));
