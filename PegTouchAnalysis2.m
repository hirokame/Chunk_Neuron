...
if FTselect==2;Duration=OneTurnTime*0.4;%All touches
    ...
    for n=1:12%PegTouchCell 1-12はRpeg 13-24はLpeg
...
%         RpegTouchCell{n+6}=[RpegTouchCell{n+6};PegTouchCell{n}(((TurnMarkerTime(1)-OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<((TurnMarkerTime(1)-OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration)))];
        LpegTouchCell{n}=[LpegTouchCell{n};PegTouchCell{n+12}(((TurnMarkerTime(1)-OneTurnTime)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<((TurnMarkerTime(1)-OneTurnTime)+SliderTouch(n+12)*OneTurnTime+Duration)))];
...
%         RpegTouchTurn{n+6}=[RpegTouchTurn{n+6};-TurnMarkerTime(1)+PegTouchCell{n}(((TurnMarkerTime(1)-OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<((TurnMarkerTime(1)-OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration)))];
        LpegTouchTurn{n}=[LpegTouchTurn{n};-TurnMarkerTime(1)+PegTouchCell{n+12}(((TurnMarkerTime(1)-OneTurnTime)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<((TurnMarkerTime(1)-OneTurnTime)+SliderTouch(n+12)*OneTurnTime+Duration)))];
...
%             RpegTouchCell{n+6}=[RpegTouchCell{n+6};PegTouchCell{n}((TurnMarkerTime(m)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<=(TurnMarkerTime(m)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration)))];        
            LpegTouchCell{n}=[LpegTouchCell{n};PegTouchCell{n+12}((TurnMarkerTime(m)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<(TurnMarkerTime(m)+SliderTouch(n+12)*OneTurnTime+Duration)))];
...
%             RpegTouchTurn{n+6}=[RpegTouchTurn{n+6};-TurnMarkerTime(m)+PegTouchCell{n}((TurnMarkerTime(m)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<=(TurnMarkerTime(m)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration)))];        
            LpegTouchTurn{n}=[LpegTouchTurn{n};-TurnMarkerTime(m)+PegTouchCell{n+12}((TurnMarkerTime(m)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<(TurnMarkerTime(m)+SliderTouch(n+12)*OneTurnTime+Duration)))];
...
%         RpegTouchCell{n+6}=[RpegTouchCell{n+6};PegTouchCell{n}(((TurnMarkerTime(length(TurnMarkerTime))+OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<((TurnMarkerTime(length(TurnMarkerTime))+OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration)))];
        LpegTouchCell{n}=[LpegTouchCell{n};PegTouchCell{n+12}(((TurnMarkerTime(length(TurnMarkerTime))+OneTurnTime)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<((TurnMarkerTime(length(TurnMarkerTime))+OneTurnTime)+SliderTouch(n+12)*OneTurnTime+Duration)))];
...
%         RpegTouchTurn{n+6}=[RpegTouchTurn{n+6};-TurnMarkerTime(length(TurnMarkerTime))+PegTouchCell{n}(((TurnMarkerTime(length(TurnMarkerTime))+OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<((TurnMarkerTime(length(TurnMarkerTime))+OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration)))];
        LpegTouchTurn{n}=[LpegTouchTurn{n};-TurnMarkerTime(length(TurnMarkerTime))+PegTouchCell{n+12}(((TurnMarkerTime(length(TurnMarkerTime))+OneTurnTime)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<((TurnMarkerTime(length(TurnMarkerTime))+OneTurnTime)+SliderTouch(n+12)*OneTurnTime+Duration)))];
...
elseif FTselect==1;Duration=OneTurnTime*0.5;%1st touch

    for n=1:12%PegTouchCell 1-6はRpeg 7-12はLpeg
...
%             RpegTouchCell{n+6}=[RpegTouchCell{n+6};min(PegTouchCell{n}((TurnMarkerTime(m)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<=(TurnMarkerTime(m)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration))))];        
            LpegTouchCell{n}=[LpegTouchCell{n};
                min(PegTouchCell{n+12}((TurnMarkerTime(m)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<(TurnMarkerTime(m)+SliderTouch(n+12)*OneTurnTime+Duration))))];
...
%             RpegTouchTurn{n+6}=[RpegTouchTurn{n+6};-TurnMarkerTime(m)+min(PegTouchCell{n}((TurnMarkerTime(m)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<=(TurnMarkerTime(m)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration))))]; 
            ...
            LpegTouchTurn{n}=[LpegTouchTurn{n};-TurnMarkerTime(m)+min(PegTouchCell{n+12}((TurnMarkerTime(m)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<(TurnMarkerTime(m)+SliderTouch(n+12)*OneTurnTime+Duration))))];
...
%         RpegTouchCell{n+6}=[RpegTouchCell{n+6};min(PegTouchCell{n}(((TurnMarkerTime(length(TurnMarkerTime)-1)+OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<((TurnMarkerTime(length(TurnMarkerTime)-1)+OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration))))];
        LpegTouchCell{n}=[LpegTouchCell{n};min(PegTouchCell{n+12}(((TurnMarkerTime(length(TurnMarkerTime)-1)+OneTurnTime)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<((TurnMarkerTime(length(TurnMarkerTime)-1)+OneTurnTime)+SliderTouch(n+12)*OneTurnTime+Duration))))];
...
%         RpegTouchTurn{n+6}=[RpegTouchTurn{n+6};-TurnMarkerTime(length(TurnMarkerTime)-1)+min(PegTouchCell{n}(((TurnMarkerTime(length(TurnMarkerTime)-1)+OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5<=PegTouchCell{n})&(PegTouchCell{n}<((TurnMarkerTime(length(TurnMarkerTime)-1)+OneTurnTime)+SliderTouch(n)*OneTurnTime+OneTurnTime*0.5+Duration))))];
        LpegTouchTurn{n}=[LpegTouchTurn{n};-TurnMarkerTime(length(TurnMarkerTime)-1)+min(PegTouchCell{n+12}(((TurnMarkerTime(length(TurnMarkerTime)-1)+OneTurnTime)+SliderTouch(n+12)*OneTurnTime<=PegTouchCell{n+12})&(PegTouchCell{n+12}<((TurnMarkerTime(length(TurnMarkerTime)-1)+OneTurnTime)+SliderTouch(n+12)*OneTurnTime+Duration))))];
...
%Pegを一本分後ろにずらしてTMとの整合性を取る。一本目のペグがTMより前に来てしまった場合。---------------------------
%pegTimeArray2Dで1本目と数えられたペグを2本目に直して、タッチセンサと整合性をとる。
...
[fig_touchR PtouchHistoCell_R]=TouchAlignByPeg(RpegTouchCell,RpegTimeArray2D,yhight);
if FigureSave==1;saveas(fig_touchR,[DrName,' ','TouchAlignRpeg.bmp']);end
[fig_touchL PtouchHistoCell_L]=TouchAlignByPeg(LpegTouchCell,LpegTimeArray2D,yhight);
...
[fig_touchR_TM,PTTMHistoCell_R]=TouchAlignByTM(RpegTouchCell,TurnMarkerTime,yhight);
[fig_touchL_TM,PTTMHistoCell_L]=TouchAlignByTM(LpegTouchCell,TurnMarkerTime,yhight);
...
for n=1:12;plot(PTTMHistoCell_R{n},'g');end;
for n=1:12;plot(PTTMHistoCell_L{n},'r');end;
hold off;
if FigureSave==1;saveas(fig_TMpegs,[DrName,' ','TM_pegRL.bmp']);end

RpegMedian=zeros(1,12);% TouchのMedian
