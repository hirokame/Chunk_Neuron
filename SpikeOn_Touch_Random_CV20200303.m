function SpikeOn_Touch_Random_CV20200303
       ...
       for j=1:length(LS1(:,1))
           ...
           if length(TrimFolder2)>22 && strcmp(TrimFolder2(end-21:end),'SpikeOnOffRtLtUnit.mat');
              ...
              for x=1:length(LtouchPosiNegaTimeUnit(:,1))
                  ...
                  if length(IndexPositive)==5 && length(IndexNegative)==0 && LtouchPosiNegaTimeUnit(x,1)~=0;
...
%              bar([MeanCVLtouchArray MeanRandomCVLtouchArray],0.5);
              ...
              %%%%%%%%%%%%%%%%%%%%%%%Rtouch%%%%%%%%%%%%%%%%%%%%%%%%
               for x=1:length(RtouchPosiNegaTimeUnit(:,1))
                  ...
                  if length(IndexPositive)==5 && length(IndexNegative)==0 && RtouchPosiNegaTimeUnit(x,1)~=0;
...
set(gca,'xticklabel',{'PeakCV','CVLtouch_SpikeOn','RandomCV'});
...
set(gca,'xticklabel',{'PeakCV','CVRtouch_SpikeOn','RandomCV'});
