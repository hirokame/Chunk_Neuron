...
bin=500; % CCresultLtouchWonの更新に必要,smoothing width
CCresultLtouchWon=hist(CrossCoL,bin);
CCresultLtouchWon=CCresultLtouchWon/sum(CCresultLtouchWon);
CCresultLtouchWon=MovWindow(CCresultLtouchWon,10); %%10msずつwindowを動かしていく？
...
[Lpks,Llocs,Lwidth,Lproms]=findpeaks(CCresultLtouchWon,'MinPeakDistance',20,'MinPeakProminence',mean(Lproms)*0.3);%%%%%%%上で計算したプロミネンスの平均＊0.3以上のピークを検出
...
[Rpks,Rlocs,Rwidth,Rproms]=findpeaks(CCresultRtouchWon,'MinPeakDistance',20,'MinPeakProminence',mean(Rproms)*0.3);%%%%%%%上で計算したプロミネンスの平均＊0.3以上のピークを検出
...
[RTroughpks,RTroughlocs,RTroughwidth,RTroughproms]=findpeaks(CCresultRtouchWonTrough,'MinPeakDistance',20,'MinPeakProminence',mean(RTroughproms)*0.3); %%%%%%%上で計算したプロミネンスの平均＊0.3以上のピークを検出
...
%%%%%%ウィンドウ１のスタートの位置を０ｓとする値に変換（LPWS(1)orRPWS(1)分、値をずらす）
LPWS1=LPWS-LPWS(1);
LPWE1=LPWE-LPWS(1);
LNWS1=LNWS-LPWS(1);
LNWE1=LNWE-LPWS(1);
...
save(filename,'RtouchSpikeOnUnit','LtouchSpikeOnUnit','RtouchSpikeOffUnit','LtouchSpikeOffUnit','LPWS','LPWE','RPWS','RPWE','LNWS','LNWE','RNWS','RNWE'...
    ...
    ,'RpegTouchallWon','LpegTouchallWon','SpikeArrayWon','RpegTouchallWon','LpegTouchallWon','CCresultRtouchWon','CCresultLtouchWon','RtouchWindowTrough');
