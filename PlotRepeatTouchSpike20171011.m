function PlotRepeatTouchSpike20171011(TouchIntervalSet,SpikeA,NN,PegTouchAll,fname,tfile)
...
if max(max(PeakMean))>0
    ...
    title([fname,'  ',tfile(1:11),', Firing Rate at intervals ',int2str(NN),'ms']);
    for n=1:length(NN)
    ...
    subplot(2,length(NN)+1,length(NN)+n+2);
        errorbar(PeakMeanAve,PeakMeanSem,'b');
        ylim([0 max(max(PeakMean))*1.2]);
