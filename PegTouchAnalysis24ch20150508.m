...
global SliderTouch OneTurnTime RefPeriod RpegMedian LpegMedian RLpegMedian RLpegName DrName fig_touchR fig_touchL fig_touchR_TM fig_touchL_TM fig_TMpegs...
    ...
    RpegTimeArray2D_turn LpegTimeArray2D_turn fig_TouchRL fig_TouchR fig_TouchL
...
%　0の場合は直前のペグタッチ＋１の値を入れておく％％％％％％％％％％％％％％％％％％％％％％％％％％％％％％％
R0=find(isnan(MedianRtouch));
if ~isempty(R0)
    for n=1:length(R0)
        if R0(n)==1
            MedianRtouch(R0(n))=MedianRtouch(R0(n)+1)-1;%OneTurnTime*0.01;
        else
            MedianRtouch(R0(n))=MedianRtouch(R0(n)-1)+1;
        end
    end
end
...
if ~isempty(L0)
    for n=1:length(L0)
        if L0(n)==1
            MedianLtouch(L0(n))=MedianLtouch(L0(n)+1)-1;%OneTurnTime*0.01;
        else
            MedianLtouch(L0(n))=MedianLtouch(L0(n)-1)+1;
        end
    end
end

% RpegS=1;LpegS=1;
% while  RpegS==1 ||  LpegS==1
...
for n=1:12
    RpegTouchTurnSorted{n}=RpegTouchTurn{indR(n)};
    LpegTouchTurnSorted{n}=LpegTouchTurn{indL(n)};
    
    RpegTouchCellSorted{n}=RpegTouchCell{indR(n)};
    LpegTouchCellSorted{n}=LpegTouchCell{indL(n)};
    
    RpegTouchMatrixSorted(:,n)=RpegTouchMatrix(:,indR(n));
    LpegTouchMatrixSorted(:,n)=LpegTouchMatrix(:,indL(n));
    n
    indR(n)
    
    RpegTouchTurnMatrixSorted(:,n)=RpegTouchTurnMatrix(:,indR(n));
    LpegTouchTurnMatrixSorted(:,n)=LpegTouchTurnMatrix(:,indL(n));
end
...
for n=1:12
    ...
    text(100,6,RpegNameSorted(n,:));
    text(RpegMedianSorted(n),2,'l','FontSize',16,'color',[0 1 0]);
end
...
for n=1:12
    ...
    text(100,6,LpegNameSorted(n,:));
    text(LpegMedianSorted(n),2,'l','FontSize',16,'color',[1 0 0]);
end
...
[fig_touchR PtouchHistoCell_R RpegTouchBypegCell]=TouchAlignByPeg(RpegTouchCellSorted,RpegTimeArray2Dmod,yhight);
if FigureSave==1;saveas(fig_touchR,[DrName,' ','TouchAlignRpeg.bmp']);end
[fig_touchL PtouchHistoCell_L LpegTouchBypegCell]=TouchAlignByPeg(LpegTouchCellSorted,LpegTimeArray2Dmod,yhight);
