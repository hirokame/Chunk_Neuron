function varargout = WheelAnalyzer(varargin)
...
%     fig_DrinkMark,fig_FloorMark,fig_WaterOn,fig_DrinkOn,fig_Floor
...
[RpegTimeArray,LpegTimeArray,RpegTimeArray2D,LpegTimeArray2D,OneTurnTime TurnMarkerTime]=DataLoad(PegOffset,dpath,fname);
%disp(RpegTimeArray);
%disp(LpegTimeArray);
...
%        SliderTouch=[0.9300 0.0200 0.1100 0.1900 0.2800 0.3600 0.9600 0.0500 0.1400 0.2200 0.3100 0.3900];
       SliderTouch=[0.900 0.000 0.0345 0.1360 0.1890 0.2860 0.8460 0.000 0.0850 0.1237 0.2100 0.3080];
...
TouchDividerAnalyzer(PegTouchTurn,OneTurnTime,SliderTouch);
...
global RpegTouchCell LpegTouchCell RLpegTouchCell PegDetachCell CrossVar CrossVarInv RpegTouchCell LpegTouchCell...
    OneTurnTime DrName RpegTouchTurn LpegTouchTurn FTselect fig_CrossVarInv_CLmap Num_SmallVar ChunkLength ChunkEnd ShowFig
...
global SliderTouch PegTouchCell TurnMarkerTime OneTurnTime RpegTimeArray2D LpegTimeArray2D RpegTouchCell LpegTouchCell RLpegTouchCell...
    ...
    RmedianPTTM LmedianPTTM patternValue
...
[RpegTouchCell,LpegTouchCell,RLpegTouchCell,RLpegName,RLpegMedian,RpegTouchTurn,LpegTouchTurn]=...
    PegTouchAnalysis(PegTouchCell,SliderTouch,TurnMarkerTime,OneTurnTime,RpegTimeArray2D,LpegTimeArray2D,FTselect);
...
global dpath dpath2 fname RefPeriod OneTurnTime PegTouchCell fname DrName FTselect PegTouchTurn SliderTouch...
    ...
    fig_OnWater fig_DrinkMark fig_NumDrink fig_FloorMark fig_NumFloorTouch fig_WaterOnLen fig_DrinkOnCount fig_FloorTouchCount
...
close(fig_DrinkOn);
% close(fig_Floor);%,...
%                     close(fig_touchR,fig_touchL,fig_touchR_TM,fig_touchL_TM,fig_TMpegs);%,...
                    close(fig_VM);
%                     close(fig_CrossVarInv_CLmap);
                    close(fig_VarInvRe_CLmap48);
                    close(fig_OnWater);
%                     close(fig_DrinkMark);%,...
                    close(fig_NumDrink);
%                     close(fig_NumFloorTouch);
%                     close(fig_CrossVarRL);
%                     close(fig_TouchR);
%                     close(fig_TouchL);
%                     close(fig_TouchRL);
%                     close(fig_VM2);%,...
%                     close(fig_CrossVar_CLmap2,fig_WaterOnLen,fig_DrinkOnCount,fig_FloorTouchCount,fig_CrossVar_CLmap,fig_CrossVar);%,...
%     close(fig_OnWater);
%     close(fig_NumDrink);
%     close(fig_WaterOnLen);
%     close(fig_DrinkOnCount);%,...
...
fig_Raster=figure('Position',[1 400 1400 575]);hold on
...
fig_Raster=figure('Position',[1 1 1400 975]);hold on
if ~isempty(R1select);Raster1=raster(R1select,TurnMarkerTime);if length(Raster1)>1;text(Raster1(:,1),Raster1(:,2),'l','FontSize',14,'color',[0 1 0]);end;end
if ~isempty(R2select);Raster2=raster(R2select,TurnMarkerTime);if length(Raster2)>1;text(Raster2(:,1),Raster2(:,2),'l','FontSize',14,'color',[1 0 1]);end;end
if ~isempty(R3select);Raster3=raster(R3select,TurnMarkerTime);if length(Raster3)>1;text(Raster3(:,1),Raster3(:,2),'>','FontSize',12,'color',[0 0 1]);end;end
if ~isempty(R4select);Raster4=raster(R4select,TurnMarkerTime);if length(Raster4)>1;text(Raster4(:,1),Raster4(:,2),'>','FontSize',12,'color',[1 0 0]);end;end
