function RLphase20191024
%仮説６：Ltの間のRtの位相を用いた予測
%RLphase20190517からの変更点：1000ビンに直すところjMaxの値を小数第一位までに変更、ヒルベルト変換を使わないようにした
global ...
    ...
    fig FLName EnveRLPhaseTM EnveCCresultSpikePro meanKldistDiff KldistSame  dpath  KLD6 meanXaxiserror6 Envedist6 meanXaxiserror Envedist
...
% dpath3
% 
trimFLName=strtrim(FLName);
MD=['RLphase20190517',trimFLName((end-4):end)];
mkdir(MD);
% LS=ls;
% for i=1:length(LS(:,1))
%     FL=strtrim(LS(i,:));
...
%                             OneStepSpike=OneStepSpike+OneStepSpike1;
                        end
                    end
                end
                break;
               ...
               bin2=LpegTouchallWon98(i+1)-LpegTouchallWon98(i);
               OneStepSpike=OneStepSpike+zeros(1,bin2); 
%                OneStepSpike=OneStepSpike+OneStepSpike1;
            end
        end
    end
...
S=sum(RLPhaseTM1000);
RLPhaseTM1000Pro=RLPhaseTM1000/S;
% figure
% plot(linspace(0,OneTurnTime98*2,length(RLPhaseTM1000Pro)),RLPhaseTM1000Pro,'color','b');
% axis([0 OneTurnTime98*2 0 max(RLPhaseTM1000Pro)*1.1]);
% 平均化フィルター
...
% % 包絡線
% x = hilbert(avgRLPhaseTM);%%%%%%%%%%%%%%%%%%%%%%%%%%ヒルベルト変換は使用しない20191024                                                    
...
% % 包絡線
% x = hilbert(avgCCresultSpikePro);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%ヒルベルト変換は使用しない20191024                                                 
...
x=figure;%off
subplot(2,1,2)  %off  20200210 つけたし
...
meanXaxiserror6=meanXaxiserror;
Envedist6=Envedist;
