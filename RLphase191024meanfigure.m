function RLphase191024meanfigure
%RLphase191024meanのfigure作成用
...
% subplot(2,1,2)  
[PeakEnveRLPhaseTM IndexEnveRLPhaseTM]=findpeaks(EnveRLPhaseTM,'MinPeakDistance',30);
[PeakEnveCCresultSpikePro IndexEnveCCresultSpikePro]=findpeaks(EnveCCresultSpikePro,'MinPeakDistance',30);
%%%20191126変更点
fig1=figure
X=linspace(0,OneTurnTime98*2,length(EnveRLPhaseTM));
plot(linspace(0,OneTurnTime98*2,length(EnveRLPhaseTM)),EnveRLPhaseTM,'color','r'); hold on
plot(X(IndexEnveRLPhaseTM),PeakEnveRLPhaseTM,'color','r');
fig2=figure
Y=linspace(0,OneTurnTime98*2,length(EnveCCresultSpikePro));
plot(linspace(0,OneTurnTime98*2,length(EnveCCresultSpikePro)),EnveCCresultSpikePro,'color','k');hold on
plot(Y(IndexEnveCCresultSpikePro),PeakEnveCCresultSpikePro,'color','k');
...
fig3=figure
plot(linspace(0,1,length(PeakEnveRLPhaseTM)),PeakEnveRLPhaseTM,'color','r');
fig4=figure
plot(linspace(0,1,length(PeakEnveCCresultSpikePro)),PeakEnveCCresultSpikePro,'color','k');
trimtfile=strtrim(tfile);

Oldfig=cd;
cd('C:\Users\C238\Desktop\terashita\figure2')

fignameX=[trimtfile(1:9),'EnveRLPhaseTM','.bmp'];
fignameY=[trimtfile(1:9),'EnveCCresultSpikePro','.bmp'];
fignameZ=[trimtfile(1:9),'PeakEnveRLPhaseTM','.bmp'];
fignameW=[trimtfile(1:9),'PeakEnveCCresultSpikePro','.bmp'];
saveas(fig1,fignameX);
saveas(fig2,fignameY);
saveas(fig3,fignameZ);
saveas(fig4,fignameW);
...
figure
plot(linspace(0,OneTurnTime98*2,length(RAllTouchPhaseTM)),RAllTouchPhaseTM,'color','b');
figure
plot(linspace(0,OneTurnTime98*2,length(LAllTouchPhaseTM)),LAllTouchPhaseTM,'color','r')
if max(max(RAllTouchPhaseTM),max(LAllTouchPhaseTM))>0;
axis([0 OneTurnTime98*2 0 max(max(RAllTouchPhaseTM),max(LAllTouchPhaseTM))*1.1]);
