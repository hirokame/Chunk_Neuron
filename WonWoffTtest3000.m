...
if ~isempty(SpikeArray);
    ...
    if isempty(RasterData)==0;
        if render == 1
            ...
            axis([-3000 3000 0 length(WaterOnArrayOriginal)]);
        end
        WonPrePostkaisu=[];
        for i=1:length(WaterOnArrayOriginal)
    ...
    if isempty(RasterData)==0;
        if render == 1
            scatter(RasterData(:,1),RasterData(:,2),sz,'ko','filled');hold on
            axis([-3000 3000 0 length(WaterOffArrayOriginal)]);
        end
        WoffPrePostkaisu=[];
        for i=1:length(WaterOffArrayOriginal)
        ...
        if render == 1
            title('Woff');
            xlabel([trimtfile,'  hpre=',num2str(Woffhpre),'  ppre=',num2str(ppre),'  hpost=',num2str(Woffhpost),'  ppre=',num2str(ppost)]);
            hold off
        end
    end
    Folder=['WOnWofftest'];
...
save(FLName,'Wonhpre3000','Woffhpost3000','-append');
cd(Old)
