function matsuda20190913RepeattouchW468
...
for n=1:(length(SpikeArrayWon))
    ...
    for j=1:(length(RpegTouchallWon))    
            if  RpegTouchallWon(j)>SpikeArrayWon(n)+RWindowS1&SpikeArrayWon(n)+RWindowE1>RpegTouchallWon(j)|...
                RpegTouchallWon(j)>SpikeArrayWon(n)+RWindowS2&SpikeArrayWon(n)+RWindowE2>RpegTouchallWon(j)|...
                RpegTouchallWon(j)>SpikeArrayWon(n)+RWindowS3&SpikeArrayWon(n)+RWindowE3>RpegTouchallWon(j)
                CountR6=CountR6+1;
            end
    end
...
for n=1:(length(SpikeArrayWon))
    ...
    for j=1:(length(RpegTouchallWon))    
            if  RpegTouchallWon(j)>SpikeArrayWon(n)+RWindowS1&SpikeArrayWon(n)+RWindowE1>RpegTouchallWon(j)|...
                RpegTouchallWon(j)>SpikeArrayWon(n)+RWindowS2&SpikeArrayWon(n)+RWindowE2>RpegTouchallWon(j)|...
                RpegTouchallWon(j)>SpikeArrayWon(n)+RWindowS3&SpikeArrayWon(n)+RWindowE3>RpegTouchallWon(j)|...
                RpegTouchallWon(j)>SpikeArrayWon(n)+RWindowS4&SpikeArrayWon(n)+RWindowE4>RpegTouchallWon(j)
                CountR8=CountR8+1;
            end
    end
...
figname=['Repeattouch',trimtfile,pegpatternname,'.bmp'];
saveas(fig2,figname);
...
for n=1:(length(SpikeArrayWon))
    ...
    for j=1:(length(RpegTouchallWon))    
            if  RpegTouchallWon(j)>SpikeArrayWon(n)+RWindowS2&SpikeArrayWon(n)+RWindowE2>RpegTouchallWon(j)|...
                RpegTouchallWon(j)>SpikeArrayWon(n)+RWindowS4&SpikeArrayWon(n)+RWindowE4>RpegTouchallWon(j)
                CountR=CountR+1;
            end
    end
...
fig3=figure;
hist(CountArray,xbins4,'barwidth',0.1)
trimtfile=strtrim(tfile);
figname=['Repeattouchbs',trimtfile,pegpatternname,'.bmp'];
saveas(fig3,figname);
close(fig1);
close(fig2);
close(fig3);
...
%     end
%    
%     countarrayl1=[countarrayl1 CountL1];
%     countarrayl2=[countarrayl2 CountL2];
%     Count2=CountL2+CountR2;
%     CountArrayL2=[CountArrayL2 Count2];
% end
% 
% WL0=length(find(CountArrayL2==0));
% WL1=length(find(countarrayl1==1));
% WL2=length(find(countarrayl2==1));
% WL12=length(find(CountArrayL2==2));
% 
% y=[WL0, WL1,WL2,WL12];
% x=0:1:4;
% 
% fig4=figure;
% bar(x,y)
% figname=['RepeattouchWL2',pegpatternname,'.bmp'];
% saveas(fig4,figname);
% 
% fig2=figure;
% subplot(3,1,1);
