function [RpegTouchMatrix LpegTouchMatrix RpegTouchTurnMatrix LpegTouchTurnMatrix...
    ...
    ]=PegTouchAnalysis24chVolt(TouchTiming,TurnMarkerTime,OneTurnTime,RpegTimeArray2D,LpegTimeArray2D,Title)
%VファイルからVoltageAnalysis20170719で計算されたTouchTiming, DetachTimingを用いて
%RpegTouchCellなどを作成
%Touch, Detachのヒストグラムを表示
...
global SliderTouch OneTurnTime RefPeriod RpegMedian LpegMedian RLpegMedian RLpegName DrName fig_touchR fig_touchL fig_touchR_TM fig_touchL_TM fig_TMpegs...
    PtouchHistoCell_R PtouchHistoCell_L WaterOn_percent  FigureSave RpegTimeArray2D LpegTimeArray2D...
    RpegTouchBypegCell  LpegTouchBypegCell RmedianPTTM LmedianPTTM ShowFig patternValue...
    RpegNameSorted LpegNameSorted RLpegNameSorted...
    RpegMedianSorted LpegMedianSorted...
...
%タッチの順番ではなくてペグの順番にする
RLpegMedian=[RpegMedian LpegMedian];
[RLpegMedian RLpegIndex]=sort(RLpegMedian,'ascend');
RLpegNameSorted=RLpegName(RLpegIndex,:);
...
for n=1:RpNum+LpNum
    RLpegTouchTurnSorted{n}=RLpegTouchTurn{RLpegIndex(n)};
    RLpegTouchCellSorted{n}=RLpegTouchCell{RLpegIndex(n)};
    RLpegTouchMatrixSorted(:,n)=RLpegTouchMatrix(:,RLpegIndex(n));
    RLpegTouchTurnMatrixSorted(:,n)=RLpegTouchTurnMatrix(:,RLpegIndex(n));
end
...
if FigureSave==1;saveas(fig_touchL_TM,[DrName,' ','touchL_TM.bmp']);end
    ...
    Max=max([Rmax Lmax]);
    axis([0 ceil(OneTurnTime/10) 0 Max+2]);title(Title);
