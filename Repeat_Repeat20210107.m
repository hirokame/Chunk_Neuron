...
for i=1:length(LS1(:,1)) %1列目で固定、行を動かす。各マウスでイテレーションを回す。
    ...
    if ~strcmp(trimFolder,'.') && ~strcmp(trimFolder,'..') && isempty(strfind(trimFolder,'.bmp')) && isempty(strfind(trimFolder,'.xlsx')) &&...
            ~strcmp(trimFolder,'CCSfigure') && isempty(strfind(trimFolder,'SpikeFormAnalysis')) && isempty(strfind(trimFolder,'Count')) &&...
            isempty(strfind(trimFolder,'widthPtoT')) && ~strcmp(trimFolder,'CCSfigure-Lt') && ~strcmp(trimFolder,'CCSfigure-RLt') && ~strcmp(trimFolder,'CCSfigure-Rt') &&...
            isempty(strfind(trimFolder,'Hirokane')) &&isempty(strfind(trimFolder,'Window')) &&isempty(strfind(trimFolder,'Chunk'))
...
%                                 saveas(gcf,figname)
                                ...
                                save(filename,'LtPositiveFiringRateArray_gt4','LtPositiveFiringRateArray_lt3','RtPositiveFiringRateArray_gt4','RtPositiveFiringRateArray_lt3'...
                                    ,'LtPositiveFiringRateArray_devide','RtPositiveFiringRateArray_devide')
